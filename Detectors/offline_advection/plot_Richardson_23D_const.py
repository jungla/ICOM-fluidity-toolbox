...
Rich = RD_2D[:,0]/ttime**3/drateDt[depths[0]]
print '2D 5m: mean', np.mean(Rich), 'std', np.std(Rich)
R2D1, = ax1.semilogx(ttime,Rich,'k',linewidth=2)

Rich = RD_3D[:,0]/ttime**3/drateDt[depths[0]]
print '3D 5m: mean', np.mean(Rich), 'std', np.std(Rich)
R3D1, = ax1.semilogx(ttime,Rich,'k--',linewidth=2)
...
plt.legend((R2D1,R3D1),('2D 5m','3D 5m'),loc=2,fontsize=16,ncol=2)
