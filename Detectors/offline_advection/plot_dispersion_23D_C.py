...
for z in range(len(depths)):
 ...
 # save to file
 f = open('RD_3D_'+label+'.csv','w')
 writer = csv.writer(f)
 writer.writerow(('time',depths[0],depths[1],depths[2]))
 for i in range(len(time)-1):
  writer.writerow((time[i],RD_3D[i,0],RD_3D[i,1],RD_3D[i,2]))
 f.close()
...
plt.legend((p_2D,p_2D5,p_2D17,p_3D,p_3D5,p_3D17),('2D 5m','2D 10m','2D 17m','3D 5m','3D 10m','3D 17m'),loc=4,fontsize=16)
#plt.legend((p_2D,p_2D5,p_2D11,p_2D17,p_2D26,p_3D,p_3D5,p_3D11,p_3D17,p_3D26),('2D 1m','2D 5m','on 11m','2D 17m','on 26m','3D 1m','3D 5m','off 11m','3D 17m','off 26m'),loc=2,fontsize=12)
...
new_x = np.linspace(np.log10(1440),4.2,10)
y = 4 -3.*new_x[0] + 3.*new_x
plt.plot(new_x,y,'k',linewidth=1.5)
plt.text(4.3,y[-1],'3',fontsize=16)
y = 4 -2.*new_x[0] + 2.*new_x
plt.plot(new_x,y,'k',linewidth=1.5)
plt.text(4.3,y[-1],'2',fontsize=16)
y = 4 -1.*new_x[0] + 1.*new_x
plt.plot(new_x,y,'k',linewidth=1.5)
...
plt.legend((p_3D,p_3D5,p_3D17),('3D 5m','3D 10m','3D 17m'),loc=4,fontsize=16)
#plt.xlim((0,5))
#plt.xlim((0.1,6))
plt.xlim((np.log10(1000),np.log10(72*3600)))
plt.ylim((2-.5,8))

#ind = np.linspace(2,14,13)*86400    # the x locations for the groups
ind = np.linspace(0,288,6)*3600
...
plt.ylabel(r'$log(\sigma^2_D)$ $[m^2]$',fontsize=20)
#plt.ylabel('Relative Dispersion [m^2]')
plt.legend((p_2D,p_2D5,p_2D17),('2D 5m','2D 10m','2D 17m'),loc=4,fontsize=16)
#plt.xlim((2,5))
plt.xlim((np.log10(1000),np.log10(72*3600)))
plt.ylim((2-.5,8))
...
timeD = []
with open('drate_B25.csv', 'rb') as csvfile:
...
vtime = time - time[0]
vtime = vtime[5:]
vtime = vtime[(vtime > 0.1*86400) * (vtime < 86400)]

cD_2D = np.power(10,RD_2D[:,0])
Val = cD_2D[(vtime > 0.1*86400) * (vtime < 86400)]
...
Val = cD_3D[(vtime > 0.1*86400) * (vtime < 86400)]
f = interpolate.interp1d(timeD, drate[:,1])
drateT = f(vtime)   # use interpolation function returned by `interp1d`
Rich = Val/vtime**3/drateT
...
plt.legend((R2D1,R2D5,R2D17,R3D1,R3D5,R3D17),('2D 5m','2D 10m','2D 17m','3D 5m','3D 10m','3D 17m'),loc=3,fontsize=16)
...
#plt.legend((p_2D,p_2D5,p_2D17,p_3D,p_3D5,p_3D17),('2D 1m','2D 5m','2D 17m','3D 1m','3D 5m','3D 17m'),loc=4,fontsize=12)
plt.legend((p_2D,p_2D5,p_2D17,p_3D,p_3D5,p_3D17),('2D 5m','2D 10m','2D 17m','3D 5m','3D 10m','3D 17m'),loc=4,fontsize=16)
