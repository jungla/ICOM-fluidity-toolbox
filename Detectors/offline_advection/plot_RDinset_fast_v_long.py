...
new_x = np.linspace(np.log10(45000),5.2,10)
new_1x = np.linspace(4.9,5.45,10)
y3 = -0.1 -3.*new_x[0] + 3.*new_x
#y2 = 0.5 -2.*new_x[0] + 2.*new_x
y1 = 2.1 -1.*new_1x[0] + 1.*new_1x

plt.plot(new_1x,y1,'k',linewidth=1.5)
#plt.plot(new_x,y2,'k',linewidth=1.5)
plt.plot(new_x,y3,'k',linewidth=1.5)
plt.text(4.98,0.5,'$t^3$',fontsize=20)	
plt.text(5.15,2.5,'$t$',fontsize=20)	
...
plt.ylabel(r'$\sigma^2_{D_z}$ $[m^2]$',fontsize=24)
plt.xlim((np.log10(1440),np.log10(24*3600*7)))
...
for i in range(-1,4,1):
 M_yticks_labels.append(np.power(10.,i))
 M_yticks.append(i)
 if i < 3:
...
#vy = [0.1,0.5,1,5,10,50,100,500,1000]# np.linspace(-1,3,9)
#vyl = np.log10(vy)
#plt.yticks(vyl,vy,fontsize=14)
...
plt.tight_layout()
plt.savefig('./plot/'+label+'/RDv_'+label+'.eps')
print       './plot/'+label+'/RDv_'+label+'.eps'
plt.close()
