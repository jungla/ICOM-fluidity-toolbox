...
filename2D_B = './csv/RD_2D_'+label+'.csv'
filename3D_B = './csv/RD_3D_'+label+'.csv'

time2D_B, RD_2D_B = lagrangian_stats.read_dispersion(filename2D_B)
time3D_B, RD_3D_B = lagrangian_stats.read_dispersion(filename3D_B)

time = time2D_B[:]  - time2D_B[0]
time = time3D_B[:]  - time3D_B[0]

RD_2D = np.log10(RD_2D_B)
RD_2D = RD_2D[1:]
RD_3D = np.log10(RD_3D_B)
RD_3D = RD_3D[1:]

x = np.log10(time)
x = x[1:]

if label == 'm_25_1b_particles':
 print dim
...
if label == 'm_25_2b_particles':
 new_x = np.linspace(np.log10(50000),5.35,10)
 y = 4.5 -3.*new_x[0] + 3.*new_x
 plt.plot(new_x,y,'k',linewidth=1.5)
 plt.text(5.0,y[-8],'$t^3$',fontsize=16)

p_2D, = plt.plot(x,RD_2D[:,0],color=[0,0,1],linewidth=2)
p_3D, = plt.plot(x,RD_3D[:,0],color=[1,0,0],linewidth=2)
plt.legend((p_2D,p_3D),('2D 5m','3D 5m'),bbox_to_anchor=(1., 0.023),loc=4,fontsize=14)
...
#RD_2D_B[:,0] = RD_2D_B[:,0]/(time**3)
#RD_2D = np.log10(RD_2D_B)
#RD_2D = RD_2D[1:]
#RD_3D_B[:,0] = RD_3D_B[:,0]/(time**3)
#RD_3D = np.log10(RD_3D_B)
#RD_3D = RD_3D[1:]
#
...
plt.tight_layout()
plt.savefig('./plot/'+label+'/RD_23D_'+label+'.eps')
print       './plot/'+label+'/RD_23D_'+label+'.eps'
plt.close()
