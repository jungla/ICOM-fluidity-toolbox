...
filename2D = './traj/traj_'+exp+'_60_500_2D_big.csv' 
filename3D = './traj/traj_'+exp+'_60_500_3D_big.csv'

tt = 500-60 #+ 24-48 included
...
x0P = range(3500,4510,10)
y0P = range(3500,4510,10)
...
#  plt.plot((0,8),(8,8),'k',linewidth=2)
  #
  plt.text(-3.5,10.5,str(time0[t]/3600.+48)+'h',fontsize=16)
  ...
  #
  plt.legend((s3D,s2D),('3D','2D'),fontsize=18)
...
#  plt.text(-2, -40, str(z0[z])+'m, '+str(time0[t]/3600)+'h', fontsize=18)
  ...
  plt.xticks(fontsize=16)
  plt.yticks(fontsize=16)
  plt.savefig('./plot/'+exp+'/traj_'+exp+'_z'+str(z0[z])+'_'+myfun.digit(int(time0[t]),10)+'_v.eps')
  print       './plot/'+exp+'/traj_'+exp+'_z'+str(z0[z])+'_'+myfun.digit(int(time0[t]),10)+'_v.eps'
  plt.close()
