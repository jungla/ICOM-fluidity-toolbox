...
for tday in range(60,240,2): #240
 ...
 Diff_BW.append(list(0.5*np.mean(gradBW[:5,:],0)/1440.))

 gradB,poop = np.gradient(CD_B)
 Diff_B.append(list(0.5*np.mean(gradB[:5,:],0)/1440.))
 ...
 if tday%5==0:  
  p_5_B, = plt.plot(time_B/3600.,CD_B[:,0],color=[0,0,1],linewidth=3)
#  z = 1
#  p_10_B, = plt.plot(time_B/3600.,CD_B[:,z],'--',color=[0,0,1],linewidth=3)
#  z = 2
#  p_15_B, = plt.plot(time_B/3600.,CD_B[:,z],'-.',color=[0,0,1],linewidth=3)
  
  p_5_BW, = plt.plot(time_BW/3600.,CD_BW[:,0],color=[1,0,0],linewidth=3)
#  z = 1
#  p_10_BW, = plt.plot(time_BW/3600.,CD_BW[:,z],'--',color=[1,0,0],linewidth=3)
#  z = 2
#  p_15_BW, = plt.plot(time_BW/3600.,CD_BW[:,z],'-.',color=[1,0,0],linewidth=3)
...
plt.savefig('./plot/'+label+'/CDv_time_'+label+'.eps')
print       './plot/'+label+'/CDv_time_'+label+'.eps'
plt.close()
...
print 'Kz BW mnea', np.mean(Diff_BW[:,0])
...
#plt.xlim((24,96))
#ax0.set_ylim((0.0001,0.0007))
ax0.set_ylim((0.00005,0.00035))
ax0.set_xticks(())
...
plt.tight_layout()
plt.savefig('./plot/'+label+'/Diff_CD_'+label+'.eps')
print       './plot/'+label+'/Diff_CD_'+label+'.eps'
plt.close()
