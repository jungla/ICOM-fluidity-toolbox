...
for tt in time:
 for k in range(len(depths)):
  mli_pvtu = XMLPartitionedUnstructuredGridReader( FileName=['./mli_checkpoint_'+str(tt)+'.pvtu'] )
  mli_pvtu.PointArrayStatus = ['Tracer_'+str(k+2)+'_CG']
  ...
  if rank == 0:
   ...
   for n in range(nl+ll):
    ...
    for x in range(numPoints):
     data[x] = DataSliceFile.GetPointData().GetArray('Tracer_'+str(k+2)+'_CG').GetValue(x)
  ...
  if rank == 0:
   ...
   print Tr
   fd = open('./csv/Tracer_'+label+'_Tr'+str(k+2)+'_'+str(tt)+'.csv','a')
   print Tr[:,:,:]
