...
for tt in time:
 ...
 for k in range(len(depths)):
  print 'Tracer_',k+4
  C = data.GetScalarField('Tracer_'+str(k+4)+'_CG')

  fd = open('./Tracer_'+label+'_Tr'+str(k+4)+'_'+str(tt)+'.csv','a')

  for z in range(len(layers)):
   print z
   coords_l = coords[coords[:,2]==layers[z],:]
   C_l = C[coords[:,2]==layers[z]]
   Ct = interpolate.griddata((coords_l[:,0],coords_l[:,1]),C_l,(X,Y),method='linear')
