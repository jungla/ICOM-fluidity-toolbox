...
def interp(x0,y0,z0,lo,hi,maps,U0): 
 query_points = np.zeros((len(x0),3)) # lo + np.random.uniform( size=(xp*yp*zp, 3) ) * (hi - lo)
 query_points[:,0] = x0
 query_points[:,1] = y0
 query_points[:,2] = z0
 if_U = Intergrid(U0, lo=lo, hi=hi, order=3, maps=maps)
...
def RK4(x0,y0,z0,Ut0,Vt0,Wt0,Ut1,Vt1,Wt1,lo,hi,maps,dt):
 h2 = dt/2.0
 h6 = dt/6.0
 ...
 U2 = interp(x1,y1,z1,lo,hi,maps,Ut05)
 V2 = interp(x1,y1,z1,lo,hi,maps,Vt05)
 W2 = interp(x1,y1,z1,lo,hi,maps,Wt05)
 
 x1 = x0 + h2*U2
 y1 = y0 + h2*V2
 z1 = z0 + h2*W2
 
 U3 = interp(x1,y1,z1,lo,hi,maps,Ut05)
 V3 = interp(x1,y1,z1,lo,hi,maps,Vt05)
 W3 = interp(x1,y1,z1,lo,hi,maps,Wt05)

 x1 = x0 + dt*U3
 y1 = y0 + dt*V3
 z1 = z0 + dt*W3

 U4 = interp(x1,y1,z1,lo,hi,maps,Ut1)
 V4 = interp(x1,y1,z1,lo,hi,maps,Vt1)
 W4 = interp(x1,y1,z1,lo,hi,maps,Wt1)
...
def RK4_zyx(z0,y0,x0,Ut0,Vt0,Wt0,Ut1,Vt1,Wt1,lo,hi,maps,dt):
 h2 = dt/2.0
 h6 = dt/6.0
 # linear time interpolation at t = t0 + 0.5dt
 Ut05 = (Ut0 + Ut1)*0.5
 ...
 U1 = interp(z0,y0,x0,lo,hi,maps,Ut0)
 V1 = interp(z0,y0,x0,lo,hi,maps,Vt0)
 W1 = interp(z0,y0,x0,lo,hi,maps,Wt0)

 x1 = x0 + h2*U1
 y1 = y0 + h2*V1
 z1 = z0 + h2*W1

 U2 = interp(z1,y1,x1,lo,hi,maps,Ut05)
 V2 = interp(z1,y1,x1,lo,hi,maps,Vt05)
 W2 = interp(z1,y1,x1,lo,hi,maps,Wt05)

 x1 = x0 + h2*U2
 y1 = y0 + h2*V2
 z1 = z0 + h2*W2

 U3 = interp(z1,y1,x1,lo,hi,maps,Ut05)
 V3 = interp(z1,y1,x1,lo,hi,maps,Vt05)
 W3 = interp(z1,y1,x1,lo,hi,maps,Wt05)

 x1 = x0 + dt*U3
 y1 = y0 + dt*V3
 z1 = z0 + dt*W3

 U4 = interp(z1,y1,x1,lo,hi,maps,Ut1)
 V4 = interp(z1,y1,x1,lo,hi,maps,Vt1)
 W4 = interp(z1,y1,x1,lo,hi,maps,Wt1)
