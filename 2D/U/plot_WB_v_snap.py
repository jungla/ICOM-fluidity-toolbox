...
for time in range(dayi,dayf,days):
 ...
 R = -9.81/1027*np.squeeze(np.reshape(Rho,[len(Zlist),len(Xlist),len(Ylist)]))
 ...
 v = np.linspace(-5e-3,5e-3,50)
 vl = np.linspace(-5e-3,5e-3,5)
 plt.contourf(Xlist,Zlist,W*R,50,extend='both',cmap=plt.cm.PiYG)
# plt.colorbar(ticks=vl,format='%.3e')
 ...
 plt.savefig('./plot/'+label+'/wb_'+file1+'_v_snap.eps',bbox_inches='tight')
 plt.close()
 print 'saved '+'./plot/'+label+'/wb_'+file1+'_v_snap.eps\n'
 ...
 v = np.linspace(-5e-3,5e-3,50)
 vl = np.linspace(-5e-3,5e-3,5)
 plt.contourf(Xlist,Zlist,W,50,extend='both',cmap=plt.cm.PiYG)
 #plt.colorbar(ticks=vl,format='%.3e')
 plt.colorbar(format='%.3e')
 plt.contour(Xlist,Zlist,R,10,colors='k')
 plt.xlabel('X (m)')
