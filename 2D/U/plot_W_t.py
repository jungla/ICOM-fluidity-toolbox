...
Temp2b = stat2b["BoussinesqFluid"]["Velocity_CG%3"]["l2norm"]
...
plt.legend([T50,T25],['$B50_m$ - $B10_m$','$B25_m$ - $B10_m$'],loc=4)
#plt.plot(time1, KE1/V, 'k',linewidth=1.5)
plt.xlabel("Time $[days]$",fontsize=18)
plt.ylabel("$l^2-norm$ w' $[m s^{-1}]$",fontsize=18)
plt.tight_layout()
plt.savefig('./plot/V_t_50_25_10.eps')
plt.close()
...
plt.xticks(np.linspace(0,5,6),np.linspace(0,5,6).astype(int))
plt.legend([T10],['$B10_m$'],loc=4)
#plt.xlim([0.920, 0.980])
#plt.legend([T50,T25,T10],['B50_m','B25_m','B10_m'])
#plt.plot(time1, KE1/V, 'k',linewidth=1.5)
plt.xlabel("Time $[days]$",fontsize=18)
plt.ylabel("$l^2-norm$ w $[m s^{-1}]$",fontsize=18)
...
#plt.ylim([1.48, 1.52])
plt.tight_layout()
plt.savefig('./plot/V_t_25.eps')
plt.close()
print 'saved '+'./plot/V_t_25.eps\n'
