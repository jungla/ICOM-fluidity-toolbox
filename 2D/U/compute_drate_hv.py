...
epsilon_h = np.zeros((xn_25,yn_25,zn,len(time)))
epsilon_v = np.zeros((xn_25,yn_25,zn,len(time)))
...
for t in range(len(time)):
 ...
 epsilon_h[:,:,:,t] = nu_h*2*(dUdx)**2 + nu_h*2*(dVdy)**2
 epsilon_v[:,:,:,t] = nu_v*2*(dWdz)**2

 f = open('drate_h_'+label+'_'+str(time[t])+'_3D.csv','w')
 writer = csv.writer(f)
 ...
 for k in range(zn):
  for j in range(yn_25):
   ...
   for i in range(xn_25):
    row.append(epsilon_h[i,j,k,t])
   writer.writerow(row) 
 f.close()

 f = open('drate_v_'+label+'_'+str(time[t])+'_3D.csv','w')
 writer = csv.writer(f)
