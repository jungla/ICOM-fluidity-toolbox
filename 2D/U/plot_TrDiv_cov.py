...
for time in timeT:
 ...
 fileU = path+'Velocity_CG_0_'+label_V+'_'+str(time)+'.csv' #+480
 fileV = path+'Velocity_CG_1_'+label_V+'_'+str(time)+'.csv' #+480
...
plt.plot(np.log10(timeP),corrT,'k',linewidth=2)
plt.xlabel('Time [$hr$]',fontsize=18)
plt.ylabel(r'$cov(\nabla \cdot u, C)$',fontsize=18)
labels = np.log10(np.linspace(0,24,13))
labels[0] = 0.0
#plt.xticks(np.log10(labels),labels[1:].astype(int),fontsize=16)
plt.xticks(labels,['0','2','4','6','8','10','12','','','18','','','24'],fontsize=16)
plt.yticks(fontsize=16)
