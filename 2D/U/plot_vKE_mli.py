...
KE0 = (stat0["BoussinesqFluid"]["Velocity%3"]["l2norm"])/np.sqrt((2000*2000*50))
KE1 = (stat1["BoussinesqFluid"]["Velocity%3"]["l2norm"])/np.sqrt((2000*2000*50))
...
plt.ylabel("rms Vertical Velocity $[m/s]$", fontsize=22)

plt.xticks(fontsize=18)
#plt.yticks(format='%.3e',fontsize=18)

#plt.xlim([-0.000017, 0.00004])
#plt.xticks([-0.00001, 0.00002])
plt.tight_layout()
plt.savefig('./plot/'+label1+'/KEv_t_'+label1+'.eps',bbox_inches='tight')
plt.close()
print 'saved '+'./plot/'+label1+'/KEv_t_'+label1+'.eps\n'
#
