...
for time in range(dayi,dayf,days):
 ...
 #
 file0 = 'ring_checkpoint_' + str(time) + '.pvtu'
 filepath = path+file0
...
#plt.close()
 ...
 #
 ret = []
 for k in range(-10,-900,-v_span):
  for i in np.linspace(0,400,10)**2:
   ret.append([i,k])
 ret = np.asarray(ret)
 #
 rl = np.linspace(1018, 1031,15)
...
# TOTAL flux
 ...
 plt.subplot(gs[1])
 v = np.linspace(-0.01, 0.01, 20, endpoint=True)
 vl = np.linspace(-0.01, 0.01, 5, endpoint=True)
 ...
 plt.scatter(ret[:,0],ret[:,1],marker='.',s=0.1)
 #plt.autumn()
 plt.xlabel('X (m)')
 plt.ylabel('Z (m)')
