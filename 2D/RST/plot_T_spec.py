...
label  = 'm_25_2b_day3'
label_BW = 'm_25_2b_day3'
label_B = 'm_25_1b_day3'
basename = 'mli' 
dayi  =  0
dayf  = 24  +1
days  = 12
...
for time in range(dayi,dayf,days):
 ...
 #Temperature_CG_m_6e_9.csv
 file0_B = path+'Temperature_CG_'+label_B+'_'+str(time)+'.csv'
 file0_BW = path+'Temperature_CG_'+label_BW+'_'+str(time)+'.csv'
 file1_B = 'Temperature_CG_'+label_B+'_'+str(time)
...
#   plt.close()
  ...
  print 'Cells mean size', np.mean(8000./Npks)
  print 'Cells std size', np.std(8000./Npks)
  print 'Cells max size', np.max(8000./Npks)
  print 'Cells min size', np.min(8000./Npks)


 for k in range(1):
  for j in range(len(Ylist)):
   tempfft = scipy.fftpack.fft(T_B[k,j,:]**2,xn) 
   FT_B.append(abs(tempfft)**2)
   tempfft = scipy.fftpack.fft(T_BW[k,j,:]**2,xn) 
   FT_BW.append(abs(tempfft)**2)
  for i in range(len(Xlist)):
   tempfft = scipy.fftpack.fft(T_B[k,:,i]**2,xn) 
   FT_B.append(abs(tempfft)**2)
   tempfft = scipy.fftpack.fft(T_BW[k,:,i]**2,xn) 
   FT_BW.append(abs(tempfft)**2)

  FT_B = np.asarray(FT_B)
  FT_BW = np.asarray(FT_BW)
...
#  w = scipy.fftpack.fftshift(w)
  FTp_B = np.mean(FT_B,0)/xn
  FTp_BW = np.mean(FT_BW,0)/xn

fig = plt.figure(figsize=(10,8))
pB, = plt.loglog(w[w>0], FTp_B[w>0],'r',linewidth=2)
pBW, = plt.loglog(w[w>0], FTp_BW[w>0],'b',linewidth=2)
