...
for time in range(dayi,dayf,days):
 ...
 rho_z = np.mean(np.mean(rho,axis=1),axis=1)
 plt.plot([0, 0], [min(Zlist), max(Zlist)], color='k', linestyle='--', linewidth=1)
 plt.plot(rho_z,Zlist,'k')
 plt.xlabel('<rho>')
 plt.ylabel('Z (m)')
 plt.xlim([np.min(rho_z), np.max(rho_z)])
 plt.xticks([np.min(rho_z), np.max(rho_z)])
 ...
 fig = plt.figure(figsize=(3,6))
 ax = fig.add_subplot(111)
 rho_z = np.diff(np.mean(np.mean(rho,axis=1),axis=1))/np.diff(Zlist)
 plt.plot([0, 0], [min(Zlist[1:,]), max(Zlist[1:,])], color='k', linestyle='--', linewidth=1)
 plt.plot(rho_z,Zlist[1:,],'k')
 plt.xlabel('<d(rho)/dz>')
 plt.ylabel('Z (m)')
 plt.xlim([np.min(rho_z), np.max(rho_z)])
 plt.xticks([np.min(rho_z), np.max(rho_z)])
