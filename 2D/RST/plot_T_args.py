...
KE0 = 0.5*np.sqrt(stat0["BoussinesqFluid"]["Temperature_CG"]["l2norm"])
#KE1 = 0.5*np.sqrt(stat1["BoussinesqFluid"]["Velocity%magnitude"]["l2norm"])

# plot KE
fig = plt.figure()
plt.plot(time0[:], KE0[:], color='k',linewidth=1.5)
...
plt.savefig('./plot/'+label+'/T_t_'+label+'.eps',bbox_inches='tight')
plt.close()
print 'saved '+'./plot/'+label+'/T_t_'+label+'.eps\n'
#
