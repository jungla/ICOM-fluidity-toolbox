...
mli_pvtu = XMLPartitionedUnstructuredGridReader( FileName=[path+'/mli_'+str(tt)+'.pvtu'] )
mli_pvtu.PointArrayStatus = ['Temperature_CG']
...
if rank == 0:
 ...
 for n in range(nl+ll):
  ...
  for x in xrange(numPoints):
   data[x] = DataSliceFile.GetPointData().GetArray('Temperature_CG').GetValue(x)
...
if rank == 0:
 ...
 print Tr
 fd = open('./csv/Temperature_CG_'+labelo+'_'+str(tt)+'.csv','a')
 print Tr[:,:,:]
